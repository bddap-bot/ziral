...
Frame times include whatever else the machine runs, so the benchmark measures that too. Every tenth of a second it reads how much of the CPUs it may run on went to processes other than its own, counting idle and lowered-priority time as free. A window over budget fails the build only when other work held at most a quarter of those CPUs in every tenth of a second of it. Otherwise the benchmark waits until other work has held at most a quarter for five seconds and measures that target again, for up to thirty minutes per target, after which a target still over budget fails. A window within budget passes under any load, since load only lengthens frames, so a busy machine delays a landing only when its frames went over. The quarter sits below the lightest load that cost a frame. On a quiet machine other work held about half a CPU and no window went over. With other work near a quarter throughout and briefly over a third, one web frame took 25 ms. With half, native windows went over even though a quarter of the CPUs stayed free throughout, which is why the benchmark counts other work rather than free CPUs. With three quarters, both targets went over in every window.
