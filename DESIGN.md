...
Tests cover the centre and a point near the cell corner with an atom and a machine stored in both possible construction orders, hover matching the following press, lone atoms and machines winning throughout their cells, and a replacement atom remaining targetable after stepping backward to an equal preview frame. Removing the bead-radius branch makes the corner choose the atom; restoring the second atom lookup makes hover and press disagree. Each mutation is checked red before the fix is restored green. `proofs/atom-machine-pick-92.gif` (`art/gif.sh proofs/atom-machine-pick-92.gif atom-machine-pick-92 10 1280:720:0:0`) shows the atom body moving first and the machine moving from beside it in the same cell at the shipped size.
...
The inventory. `Sim::inventory` holds a count and a cap per recipe. The palette lists the machines that have a recipe, each beside the exponential pips defined below: full ivory pips and one fractional next pip for the count, no numeral; while the row is hovered, empty brass rings continue through its whole-pip cap. A lift from the palette spends nothing, so Escape, Z and a refused drop cost nothing; the drop decrements; at zero the palette lifts nothing; deleting a placed machine returns one, past the cap when the cap is reached, and the count is never clamped. The cap is the player's and per machine: hover its row in the palette and scroll through powers of two from 1 to 256; the wheel over the palette sets caps and never zooms. A compound on the glyph while its machine's count is at the cap is backpressure: it lies there until a placement lowers the count or the wheel raises the cap, for as long as that takes, and nothing marks it. The inventory is in the sim, so a ghost frame carries the counts its ticks crafted, a scroll or a deletion at ghost[n] is applied at ghost[0] and replayed, and unpause resumes from ghost[0]'s counts. A cut returns nothing, since the machine is in the clipboard and not gone; copy and paste still make machines from nothing, and #44 decides the paste.
...
The source, first output, and bonder are the three world grants. The source proves the base atom after one 400 ms tick. From there, table-driven functions prove every recipe form from its atom inputs at the bonder and second-bond applicator, every recipe item from its form at the smallest output that contains it, amber and plum in ladder order from their converter inputs, and every atom as an attainable inventory item through the reification glyph. The proof for every recipe is generated directly from `form::RECIPES`, so a new recipe cannot exist outside the reachability suite. Typed conclusions keep an inventory item, compound notation, and atom kind distinct, and the dependency graph proves every machine, token, inventory atom, recipe compound, and atom kind without replaying an earlier construction.
...
The palette. A second column beside the machines: the step, then the tokens in tape order, each beside the same inventory pips as a machine, its cap set by the wheel as a machine's is, its picture the token as the tape draws it at tape size; the step's picture is the ivory mark the tally at the head of the strip counts ghost frames in, at twice the size so it is not read as one more pip. A click on either lifts nothing, since neither is placed. Hovering a row raises its card as before: the picture, the recipe, no words. The palette is wider, so the seam test samples the board right of it and its floor came down from 500 seams to 400 with the sampled band.
...
Tests: the complete wrap reifies either centre kind, while one missing bond or one amber outer atom leaves it untouched; an affordable double-bonded pair spends its two atoms and one sacrificial base atom and lands with its bond, while one base atom short changes nothing and shows the refusal; a paste onto an atom and a paste beyond ghost zero are refused. The recipe test holds the reification row distinct under every turn from every other recipe and inside its tier-two bound. A fragment containing a bonder, an arm at the middle of its tape and a compound writes and reads byte-equal; its paste at another cell and turn has the same canonical fragment and pays for every carried item. A bad program counter refuses a mixed fragment before any world byte changes. Machine line order survives the round trip because arm and glyph execution order is state, while a machine footprint, turn or translation beyond the coordinate range is refused before preview or placement arithmetic, and a selection that cannot be written is not cut. Dropping machine lines, sorting machine lines, resetting the program counter, omitting the compound, retaining the old buffer, accepting the bad counter, allowing an overflowing footprint, turn or translation, or deleting after a failed write makes the corresponding assertion fail. `proofs/clipboard-fragment-103.png` shows the pasted fragment at the shipped size beside the exact text that made it.
...
This is the dumbest design satisfying the directive because the inventory retains one count and one cap while one function owns their visual representation. Linear pips grouped in fives would need sixteen marks at the default cap and up to 256 at the maximum; even wrapped, their fixed 86 px tally makes the shipped palette 324 px wide. Nine ungrouped exponential pips cover the maximum in 70 px, making the same palette 292 px wide without another display path.

Tests cover 0, 1, 2, 3, 4, 5, 7 and 8 at cap 8 as 0, 1, 2, 2½, 3, 3¼, 3¾ and 4 pips; a wheel notch from 8 reaches 16 or 4 and no adjacent integer; legacy caps snap to the nearest power; and the shipped palette width falls from 324 px to 292 px at the same caps. Mutation passes restore linear cap steps, remove fractional fill, and restore the old tally width; each named test goes red before the implementation is restored.

### Pips are concentric rings
...
Every instruction uses an ivory semantic mark on a dark-brass ground, with plum moved from the ground to the unchanged edge. The pair has a WCAG-style luminance ratio of 6.44:1 from the material palette's exact sRGB values, clearing a 4.5:1 floor; amber on plum is 2.45:1. The gate samples the mark and ground from every symbol through the runtime's mipmap generation and trilinear reduction to the shipped 26 px, so geometry or antialiasing cannot hide a low-contrast pair.

This is the dumbest design satisfying the directive because it changes the shared mark and tile colors in the existing source while retaining plum as the tile boundary. Ivory on plum was the smaller edit, but its mipmapped 26 px marks measured from 4.16:1 to 4.46:1; disabling the shared mip path or thickening marks would change rendering structure or shape to rescue a color pair. Per-symbol colors would add distinctions the shapes already carry.
...
One 24-character token, generated once from the system random source with a URL-safe alphabet, is a constant in the page. At load and from one fragment-change listener, the page consumes an exact match and clears the fragment without navigation. The game then calls the inventory's one fill operation, which sets every counted item to its own cap and changes no other world state.
...
The exact report is tried against all three chain atoms in separate worlds, with amber in either bonder seat. Neither end refuses, and the middle does not refuse: each writes the same single bond. The middle then has three incident bonds because the model has no valence or per-atom bond-count rule. The only applicable refusal is the existing 256-atom compound cap: a join producing 257 atoms is rejected through the firing transaction with no event and the world byte-for-byte unchanged, while a join producing 256 atoms forms. `proofs/amber-chain-97.gif` shows the reported amber joining the free end at the shipped size.

### The test profile optimizes the crate
