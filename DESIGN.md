...
The benchmark exposed two more stalls and one steady cost. Crossing a cell boundary while panning relaid every board tile; the board now spawns only the cells entering the view and despawns only those leaving it. Sound began on any key, mouse press, touch or scroll, but the browser resumes audio only on an activating press; after a modifier or a scroll every hit queued in the suspended context until the first real press played them all at once. Sound now begins only on the presses that activate the page. The painter spawned every drawn shape as a new entity each frame and despawned it the next, so every shape paid for spawning, bounds, extraction and pipeline lookup on every frame. The painter now emits strokes, and the draw system keeps the previous frame's entities for each kind of stroke, rewriting only what changed and spawning or despawning only the difference. On the busy scene the web frame's median fell from 9.5 to 7.0 ms and its 99th percentile from 14.5 to 9.1 ms.

`ziral --analyze` reports frame-time percentiles and the count of frames over budget, for the whole record and for each mark's window. The record's frame durations are its only source; a second telemetry stream would duplicate the replay clock.

The benchmark is a recorded session of a busy scene on each target: four lines that turn source atoms into cobalt and craft it, six arms turning compounds every tick, one of every other glyph, two portals each holding a line, three pinned cards, and a camera panning four tiles either way every four seconds. `ziral --bench` renders it headless at 1280 by 720; the page loads it at `#bench` as an import and three pins through the ordinary inputs, replacing the current world. Frames run unpaced, since a vsync-paced frame lasts one refresh whatever it costs. The page runs in headless Chromium on the GPU with its module compiled before the first frame: otherwise tier-up recompiles hot functions through the first minutes and pauses the page while the browser warms up. A Space press starts sound before measuring. Each target presses M at thirty seconds and passes when that mark's window, fifteen to forty-five seconds into the session, has no frame over budget. The benchmark shares a busy machine and GPU, so a failing window is measured again, up to three windows; a change that costs frames fails all three.

`node web/bench.mjs` runs both targets against the current checkout and web build. `test-map.json` runs it after the web build for Rust sources, web files, shaders and `shell.nix`, so a change that pushes the busy scene over budget does not land.

Tests: a seal stores only the inputs appended since the previous one; a chunk leaves localStorage once IndexedDB commits it; an acknowledgment forgets only the chunks it covers, and a failed upload keeps them; a whole record saved before chunking uploads and is forgotten; the analyzer's percentiles, with a frame of exactly 1/60 s within budget; the bench world crafts at every line and turns every carousel on each of 400 ticks; sound begins on M but not on Shift, Control or Escape.
