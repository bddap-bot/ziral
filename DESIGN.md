...
Editing: a click on a machine focuses it; Z deletes it; A and D turn a glyph. An arm with focus acts now: F grab, R drop, A counterclockwise, D clockwise, X wait run on the arm at once and write nothing, so A and D on a focused arm are the rotate instructions, hand and all, with the stalls the tape would meet. The strip along the bottom lists the tapes of the arms on screen, eight at most; a click on one focuses that tape and shows a cursor. With a tape focused the same keys insert at the cursor, left and right move it, home and end jump, Z is backspace, and escape or a click elsewhere leaves; nothing runs. The controls line names the keys of whichever focus is active. Dragging a machine, or dragging from the palette, carries a preview under the pointer; A and D turn it, Z deletes it, and a release over the panels returns it to where it was lifted, or, from the palette, discards it. Pan is right or middle drag.
