...
The resonator's complete housing pulses as one body and emits sparks on its one firing event. Static glyphs draw their complete albedo through the same normal-lit material as articulated parts, using a black emission map; adding a fictitious split would put a moving ring around only one of the equal seats. The art runner follows a linked sessions directory when verifying the director transcript, with the same model checks and a regression test.
