...
### Toy 1 tells given art direction from placeholder, and draws each recipe for the painter

Directive (verbatim): "i will eventually be manually designing recipes for machines, when a recipe is manually designed, it's rendered structure should feed in as inspiration to image generation for the machines texture. (if that makes sense, don't contort our system too much to satisfy this request)

i will sometimes want to enter my own art direction for a machine, as text, to be checked into the repo. when do, it should be obvious that the art direction was intended (goes in a different struct member or is a different enum variant or something), this will lets us easily track what is still programmer-art-direction"

Two kinds of direction. A machine's entry in `art/machines/manifest.toml` carries its direction under one of two keys and exactly one: `placeholder`, what the pipeline or a programmer wrote to have something to paint from, or `given`, what a person wrote to be used as it stands. In the code that is one enum with the two variants, so a machine whose direction is unknown cannot be built and nothing downstream branches on a flag it might forget to read; the parse refuses an entry with both keys or neither, naming which. Every entry that existed is placeholder. Given text is stored and used byte-identical, quotes, breaks and trailing spaces included; the shared style text is prepended at generation and never merged into it, and the pipeline never rewrites, reflows or corrects it. `cargo run -- --plan` prints each machine with its kind and whether a recipe feeds it, and counts the placeholders, so the programmer art left is one command away: eight of eight today.
...
Three alternatives disposed. A boolean beside the string leaves the string as the thing every reader sees and the flag as the thing every writer forgets. A separate file for given direction splits one machine's record in two and makes "which file wins" a question. Describing the recipe in words, as the output tiers did until now, hands the tool a paragraph where a picture says the structure exactly, and the words go stale the day the recipe table changes while the render cannot.
...
Tests: a given direction with quotes, a backslash, a tab, a line break and trailing spaces survives a manifest write and read byte-identical and comes out of the prompt with only the shared text before it; an entry with both keys is refused naming both, one with neither is refused naming neither; `--plan` lists every machine as placeholder and counts eight, and with the arm given lists it so and counts seven; a studio run with the cleanup and the source hands the cleanup's candidates the scaffold and its recipe render, the source's the scaffold alone, the recipe sentence in the cleanup's prompt and not the source's, every relight one image, the render at the first tier's frame with the compound inside the band, and a second run paints nothing and leaves the render. Mutation-tested both ways.
...
The refusal. A short line at the drop: for each short item, its picture as the palette draws it beside `have/need`, and nothing else; it stands where the drop was refused and goes on the next press or key. It is text on screen and temporary: the directive's direction is the materialization cut short with a visual tell that the inventory failed to supply, and this line stands until that is built. It is the only text the toy puts on screen: the card carries none (#45), and the strip's arm numbers are the one other numeral. The line is drawn with the strip's text facility, one function for both, and the item's picture with the palette's, one function for both.
