...
Re-laid. Each `sim::layout` copy is one inert arm beside a first-tier output with the arm's one table recipe centred on it, so the recipe crafts on the first tick. The obsolete source, bonders, second arm and 26-step base-pair program are gone rather than left to manufacture backpressure; the `bonders` scene phases the existing bonder fixture directly. `shot::SCENES` names every scene the shot tool draws, a name off the list is refused, and a test lays out each of them and every shipped world and asks `glyph_at` for each glyph's cells.
...
Tests: a glyph dragged onto another glyph's cell pops back picked, and slid one cell over its own place lands; a picked glyph turned onto another glyph's cell keeps its turn and turned onto free cells turns; a palette glyph dropped on a glyph's cell is refused and unspent, and beside it lands and spends; a paste onto a glyph is refused and beside it lands; an arm whose hand reaches over a glyph lands; no shipped layout or scene puts two glyphs on one cell; the preloaded world crafts each centred arm recipe on its first tick; the `overlap` scene replayed headless refuses its first drop and lands its second. Mutation-tested both ways. `art/reference/proof/overlap.gif` (`gif.sh art/reference/proof/overlap.gif overlap 10 520:320:380:160`) is the `overlap` shot scene: a bonder dragged onto a second-bond, refused and returned, then set down beside it.
