...
The pipeline paints the complete machine; `art/machines/rig.sh` then splits its maps using a fixed central circle, while runtime rendering selects pre-split maps by part name. This keeps the already-judged seat layout and makes the seam between parts deterministic. Painting parts independently was rejected because separate generations have no shared geometry to guarantee that their seams, scale, and seats agree; it would add a registration problem without improving the three required motions. The automated critic judges whole candidates.
...
[art/SYMBOLS.md](art/SYMBOLS.md) records the shared tile, letters and semantic marks. The original amber-on-plum palette was replaced by the ivory-on-brass pair in the instruction-contrast entry below.
...
The shared card painter uses the same clay surface for hover and pinned cards. The earlier shared offscreen surface was replaced because enlarging a pin magnified its pixels; each card now has a camera viewport at its display resolution.

Each card has a camera viewport at its display resolution. Pins of the same item share a painted location and playback; the hover uses a separate location and playback. Each camera frames the appropriate location on the card layer. Lines keep their pixel weight as they do when the board zooms. A pin's node is only its border, ringing the viewport, and the rect a pointer tests against includes that ring. One camera painting every pin at its own scale was not possible because the card's lines are gizmos, which have no transform to scale. A shared offscreen texture holding every item's card at its largest pin's scale was rejected because a card as large as the window would exceed the texture size limit.
...
Neither B nor C beat A by one critic point on two machines. That experiment retained the recipe input; the later removal is recorded in the second-bond entry below. Body-first coverage remains a proposal, and the shared brief still constrains bodies around seat marks. The measurements do not separate those two influences.
...
Historical experiment: cropping each tile by its measured face removed flat borders but left mismatched grout at neighbouring edges. The shared-template design below replaces that measurement. `art/textures/proof/grout.png` records the earlier crop at default zoom and 4x.
...
Historical design: current card surfaces and cameras are described under Info cards, current pip geometry in [art/SYMBOLS.md](art/SYMBOLS.md), and current fixture playback under Toy 1 shows what a machine does.
...
Historical design: the free-stepping entry below removes the step item and its art; the cobalt-token entry replaces the recipe table here.
...
### Toy 1 makes amber from a compound
...
Historical art: the paid step used overlapping solid and ghost ivory circles. The free-stepping change deletes that inventory item, its picture and its count display.
...
One pure function maps an inventory count and cap to the full pips and fractional next pip the palette draws. Zero has none; each full pip doubles the represented count, and the next pip fills by progress to the next power of two. Caps are powers of two from 1 through 256. A wheel notch moves one power in either direction, and a cap loaded from older state snaps to the nearest power, with a tie going upward. The palette draws the function directly, adding empty rings through the cap only while hovered. That change retained the linear geometry, later replaced by concentric rings.
...
The concentric geometry in [art/SYMBOLS.md](art/SYMBOLS.md#inventory-pips) replaces the linear row without changing the exponential count mapping.
...
The palette in [art/SYMBOLS.md](art/SYMBOLS.md) raises nominal mark contrast from amber-on-plum’s 2.45:1 to ivory-on-brass’s 6.44:1. The existing runtime-minification gate checks the generated result at 26 px.
...
The arm has one length carried by both its recipe row and each placed arm. The three rows use lengths one, two and three; the latter rows are the stated six-cobalt, five-single-bond and seven-cobalt, six-single-bond chains. Length multiplies the facing vector, so grab and drop use the far cell, rotate sweeps the held compound around the pivot on the longer arc, and pivot rotates the held compound about the hand while the arm remains in place. The tape and every act are otherwise identical.
...
Each length is painted as one complete arm in the same dark-brass and terracotta family, with one unbroken link through its crossed cells, one pivot, and one open hand. Each keeps its own palette row, hover card and fixture while sharing the arm rules. `proofs/arm-lengths-102.gif` places all three at shipped size on one tape, grabbing, rotating and dropping at their respective reaches.
...
One native command validates a record, replays it and prints JSON. First placement counts a successful inventory placement; first bond and recipe count simulation ticks, not imported contents or preview steps. A stall counts entry into backpressure, including an observed preview; repeated blocked ticks do not inflate it. Tape edits count successful content changes, with arm identity preserved through moves and deletion of other arms. Imports introduce new arm occurrences. Hesitation is elapsed record time since the previous resolved act; hover dwell and paused time integrate frame durations. Step counts use the same clipped fifteen seconds on either side as marks. Zero-length sessions have no rate. Derived counters stay out of the input format so later analysis applies to existing records. A second telemetry stream would duplicate the replay's source of truth.

A resolved act here is a discrete recorded key, press, release, drag start, palette choice, tape cursor, cap edit, paste, card press/end/scale or wheel event, including refused attempts. Continuous pointer and card movement samples do not reset hesitation; completing a card drag does. An automated recipe is an output firing on a real simulation tick. Validation and analysis deliberately reuse input application in two simple traversals instead of adding an observer to the game.

## Portal interiors

Directive (verbatim): "let us disable pause in the overworld. "s" and "g" and "space" are freed up for some other later binding. the pause while dragging an item is still nice so we keep for now"

A portal holds a canonical Sim on its entity. Focus selects the overworld or a portal and holds the viewer's interaction state, projected frame and position in time. The existing editor borrows the selected canonical simulation; replay remains Sim::replayed. Interior playback advances the viewer's position every 400 ms, without changing the baseline. Tape edits replay from that baseline. The overworld clock advances independently, retaining only the existing pause during a held transaction.

The dumbest design separates simulation ownership from the existing viewer and borrows both for input and drawing. A global pause would stop unrelated production. A second interior stepping implementation would make identical tapes behave differently. Copying an editable baseline into the focus resource would introduce two authorities for an interior, so the editor borrows the portal's simulation directly. Session replay uses the same focus and input routing as live play.
...
Contract: delete the plum converter and its compound recipe, fixture, housing, palette entry and route. Plum remains an atom used by later recipes. Its sole early route is a resonator: two separate, unbonded amber atoms occupy its two adjacent seats together. One tick changes both to plum in place, retaining their identities and positions. Either missing input, a different kind or any bond prevents the whole reaction. Both products must be removed before another pair can fire. The resonator is crafted from a new amber-and-base compound; it needs neither plum nor cobalt to bootstrap.
...
The resonator's complete housing pulses as one body and emits sparks on its one firing event. Static glyphs draw their complete albedo through the same normal-lit material as articulated parts, with the whole housing as the firing feature; adding a fictitious split would put a moving ring around only one of the equal seats. The art runner follows a linked sessions directory when verifying the director transcript, with the same model checks and a regression test.
### Activation stays on the firing feature

The material multiplied activation energy into its warming, its normal ripple and an additive white flare over every pixel of every rig part, and masked only the glow. It also received the amber response colour as sRGB components beside albedo it samples in linear light, so the warm target was far brighter than amber. A firing arm turned its inert pivot and link pale tan until its energy decayed, and every rendered swing ended washed out.

The dumbest design multiplies the energy by the emissive mask once, where the material reads it, so warming, ripple, glow and flare all follow the mask the pipeline already derives, and the flare scales the warmed glaze instead of adding white. The response colour converts to linear at the one place the material is built. A per-part energy on the Rust side was rejected: the mask already states which pixels respond, and a second gate would let the two disagree. Damping the coefficients was rejected because the defect was placement, not strength.

`proofs/activation-mask-4244.png` is the arm at native size, at rest on the left and at full activation energy on the right: the upper row under the material before this change, the lower row under it now.

## Sixty frames on every target

Directive (verbatim): "i think current version of ziral web is dropping frames. lost it's buttery smoothness. at least consistent 60fps on all targets is a hard requirement. you have a way to collect frame time telemetry from playtests yet?"
...
The page now seals the inputs appended since the previous seal into one chunk, every five seconds and when the page hides. A chunk carries an upload batch's fields: session, build, seed, the index of its first input, and its inputs. It is written to IndexedDB, and to localStorage only until IndexedDB commits it, so a page closing mid-write keeps it and pending chunks leave localStorage to the world's save. An acknowledged chunk leaves memory and storage. On load the page restores every stored chunk before uploading, so each session uploads in order. The wasm session hands each frame's inputs to the page and keeps none. A seal serializes only the last five seconds of inputs, however long the session runs. A whole record saved before chunking uploads from its first input and is then forgotten.

The benchmark exposed two more stalls and one steady cost. Crossing a cell boundary while panning relaid every board tile; the board now spawns only the cells entering the view and despawns only those leaving it. Sound began on any key, mouse press, touch or scroll, but the browser resumes audio only on input that activates the page; after a modifier, a scroll or a touch still held, every hit queued in the suspended context until the first real press played them all at once. Sound now begins on a mouse press, a key other than a modifier or Escape, or a touch's release. The painter spawned every drawn shape as a new entity each frame and despawned it the next, so every shape paid for spawning, bounds, extraction and pipeline lookup on every frame. The painter now emits strokes, and the draw system keeps the previous frame's entities for each kind of stroke, rewriting only what changed and spawning or despawning only the difference. On the busy scene the web frame's median fell from 9.5 to 7.0 ms and its 99th percentile from 14.5 to 9.1 ms; before it, about half the web windows had a frame over budget.

`ziral --analyze` reports frame-time percentiles, the count of frames over budget, and the count longer than one and a half budgets, for the whole record and for each mark's window. A display refreshing at 60 Hz paces every frame near the budget, so there only the second count separates a dropped frame from jitter. The record's frame durations are the only source; a second telemetry stream would duplicate the replay clock.

The benchmark is a recorded session of a busy scene on each target: four lines that turn source atoms into cobalt and craft it, six arms turning compounds every tick, one of every other glyph, two portals each holding a line, three pinned cards, and a camera panning four tiles either way while zooming in by up to a fifth and back. `ziral --bench` renders it headless at 1280 by 720; the page loads it at `#bench` as an import and three pins through the ordinary inputs. While it runs, its autosave goes to a slot of its own, which the next load clears, so a reload restores the saved world. Its record is not a playtest: the page stores its chunks like any other, so the benchmark pays for storage, but never uploads them, and the next load forgets them. Frames run unpaced, since a vsync-paced frame lasts one refresh whatever it costs. The page runs in headless Chromium on the GPU with its module compiled before the first frame, so the measurement leaves out the browser's warm-up: tier-up otherwise recompiles hot functions through a page's first minutes and pauses it. A Space press starts sound before measuring. Each target presses M about thirty and sixty seconds into its recorded session and passes when both thirty-second windows around those marks have no frame over budget.

Frame times include whatever else the machine runs, so the benchmark measures that too. Every tenth of a second it reads how much of the CPUs it may run on was held by work outside its own control group, counting idle and lowered-priority time as free. A window over budget fails the build only when other work held at most an eighth of those CPUs throughout it. Otherwise the benchmark waits until other work has held at most an eighth for five seconds and measures that target again, for up to thirty minutes, after which a target still over budget fails. A window within budget passes under any load, since load only lengthens frames, so a busy machine delays a landing only when its frames went over. The eighth is measured. On a quiet machine other work held about half a CPU and no window went over. With other work on a quarter of the CPUs, one web frame took 25 ms. With half, native windows went over even though a quarter of the CPUs stayed free throughout, which is why the benchmark counts other work rather than free CPUs. With three quarters, both targets went over in every window.

`node web/bench.mjs` runs both targets against the current checkout and web build, reading the page's record through the same reader the page restores from. `test-map.json` runs it after the web build for Rust sources, web files, shaders and `shell.nix`, so a change that pushes the busy scene over budget does not land.

Tests: a seal stores only the inputs appended since the previous one; a chunk leaves localStorage once IndexedDB commits it; an acknowledgment forgets only the chunks it covers, and a failed upload keeps them; restored chunks upload from the earliest whatever order storage lists them in; a whole record saved before chunking uploads and is forgotten; the analyzer's percentiles, with a frame of exactly 1/60 s within budget and a 20 ms frame over budget but not dropped; a one-column pan relays only the cells entering and leaving the view; the bench world crafts at every line and turns every carousel on each of 400 ticks; sound begins on M and on a touch's release, but not on Shift, Control, Escape or a touch's start; a benchmark session's chunks are stored but never uploaded, and the next load forgets them.
