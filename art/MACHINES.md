...
The caption becomes `machines/NAME/prompt.txt`. `art/paint.sh` takes the prompt file, sends its caption unchanged to the image tool, then checks the recorded tool call in the session rollout and replaces the file with that caption followed by Image inputs: repository paths and SHA-256 hashes in attachment order. No inputs is explicit as none; an older call whose inputs cannot be recovered is explicit as unknown. A mismatch in prompt, input paths or input hashes, an implicit conversation-image reference, or a non-square return fails the paint; retries never stretch a return into shape. A source or cobalt-converter paint also receives its manifest reference sketch. No recipe image is rendered or attached.
