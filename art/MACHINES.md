...
3. **Judge.** The critic sees the cut sprite on board tiles at gameplay scale, magnified without smoothing, beside its scaffold. The fixed rubric ranks presence, material richness, confidence and blemishes. A separate required boolean rejects any glyph that reads as a compound, regardless of score. This gate overrides the historical rubric's exclusion of how parts connect. Arms are exempt. Missing required reply fields fail judgment. Critic calls run sequentially.
