...
out=${1:-$here/reference/proof/cards-played.png}
past_every_fixture=99
shot=$(mktemp -d)
trap 'rm -rf "$shot" "$out.part"' EXIT
cd "$here/.."
cards=()
for name in bonder second-bond arm output-1 output-2 output-3; do
  rect=$(cargo run -- --shot "$shot/$name.png" "card:$name" "$past_every_fixture" | awk '$1 == "card" { print $4 "x" $5 "+" $2 "+" $3 }' | head -1)
