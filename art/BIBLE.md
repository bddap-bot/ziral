...
**Materials and light.** Glazed clay for the board and atoms, rubbed brass for arms and bonds, matte rubber for hands. Rotatable pictures are to be painted under the one warm world light at the manifest's elevation, one relight for each of six rest facings. Once every set passes, a rest facing uses its matching painter relight and a turn crossfades the two neighbours by the turn angle. A keyboard turn uses the simulation motion's frame tween and cell-specific curve for that angle, retargeting from the drawn angle when another key arrives mid-sweep and reaching rest inside the 400 ms tick. The normal map recovered from those same edits then carries only the activation ripple and flare, never the resting light. Until that complete set lands, the existing runtime light remains the shipped look. The complete arm creeps, overshoots, and rings out in one rotation about its pivot; its painted parts add no second turn.

**View.** Every machine is drawn from directly above, the camera straight down and orthographic, so each painter relight turns about the same cell and two adjacent relights can crossfade through an arm sweep without a view change. A visible side face, a cup's far inner wall, a vanishing point are the tells, and they are judged by eye: a straight-down bowl's shaded interior is as lopsided in tone as a tilted one's, so no measurement of the capture tells them apart.
...
**UI.** Dark brass strips with ivory numerals, brass borders; the lit tape row is the same strip in lighter brass. The palette counts inventory exponentially: one full ivory pip stands for one, two full pips for two, three for four, and each further pip doubles again; counts between powers fill the next pip by their progress to the next power. While a row is hovered, empty brass rings continue through its whole-pip cap. The pips are never grouped and never become numerals. Each machine and atom picture sits on a circular clay field within its brass row because the picture is the same one painted for the world tile; an atom is the board's circular bead with its brass rim, not its square skin. Instruction tokens remain directly on the strip. On the board, an ivory hover outline surrounds the target a press would take: the bead within its circular body, or the machine at the same cell outside it. Hovering an entry raises the same clay card used by every pin, within one brass border, holding the machine's sprite, its recipe in atoms and bonds, and its test case playing on its own patch of clay, with no writing on it. The clay is the board palette surface the machines were painted to read against; there is no card tint. An instruction on a tape is one modular dark-brass square per key, with a plum edge, an ivory letter at bottom-left and an ivory semantic mark at top-right (art/SYMBOLS.md); a page of the workshop's own manual, held up while Tab is held, shows the thirteen tokens woven into flourishes beside a picture of what each does, and no other writing (art/overlay/). Every placement uses the same alpha-blended fired texture, leaving the rounded silhouette as the visible edge. The strips are furniture, not board: the strip itself never wears a state glaze; every token uses the same palette and is told apart by its generated letter and mark at tape size.

`../proofs/pip-proposals-84.png` places four unchosen exponential-pip candidates on that board and palette at the shipped pixel size. The letters identify the candidates; the marks alone show their equal caps and counts.
