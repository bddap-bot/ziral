...
for dir in "$here"/*; do
  ...
  radius=$((width < height ? width / 3 : height / 3))
  centre_x=$((width / 2))
  centre_y=$((height / 2))
  edge_x=$((centre_x + radius))
  mask=$(mktemp "$here/mask.XXXXXX.png")
  trap 'rm -f "$mask"' EXIT
  magick -size "${width}x${height}" xc:black -fill white -draw "circle $centre_x,$centre_y $edge_x,$centre_y" "$mask"
  for map in albedo normal; do
    magick "$dir/$map.png" "$mask" -compose DstIn -composite "$dir/parts/${map}-moving.png"
    magick "$dir/$map.png" \( "$mask" -negate \) -compose DstIn -composite "$dir/parts/${map}-base.png"
  done
  rm -f "$mask"
  trap - EXIT
done
